...
OUTPUT_SIMULATION_DIR = "simulari_generate" # Directorul unde vor fi salvate simulările
...
def get_quizzes_by_subject_and_type(base_dir):
    ...
    """
    quizzes = {
        "biologie": {"teorie": [], "probleme": []}, # Vom pune toate grilele bio în "teorie"
    ...
    for subject_folder_name in os.listdir(base_dir):
        ...
        if "bio" in subject_folder_name.lower():
            ...
            print(f"Avertisment: Director subiect necunoscut '{subject_folder_name}'. Ignor.")
            continue
...
def create_word_document(selected_bio_quizzes, selected_chimie_quizzes, output_path, chapter_quiz_map_lines):
    ...
    for i, quiz_info in enumerate(selected_bio_quizzes):
        try:
            ...
            document.add_paragraph(f"Eroare la adăugarea imaginii {quiz_info['path']}: {e}")
    ...
    delimiter_paragraph.runs[0].font.size = Pt(24) # Mărește fontul pentru vizibilitate
...
def create_pdf_document(selected_bio_quizzes, selected_chimie_quizzes, output_path, chapter_quiz_map_lines):
    ...
    y_position = height - (2 * inch) # Poziție de start pentru imagini, sub textul inițial
    margin = 0.5 * inch
    ...
    def add_quiz_to_pdf(quiz_info, current_y, quiz_idx):
        ...
        try:
            ...
            if current_y - img_height < margin: # Verifică dacă imaginea încape pe pagina curentă
                ...
                c.setFont("Helvetica-Bold", 10) # Resetează fontul după schimbarea paginii
                current_y -= 0.3 * inch

            c.drawImage(img_path, margin, current_y - img_height, width=img_width, height=img_height)
            current_y -= (img_height + 0.2 * inch) # Spațiu între imagini

        except Exception as e:
            print(f"Eroare la adăugarea imaginii {quiz_info['path']} în PDF: {e}")
            c.drawString(margin, current_y, f"Eroare la încărcarea imaginii: {quiz_info['path']}")
            current_y -= (0.5 * inch)
        return current_y
    ...
    start_num_chimie = len(selected_bio_quizzes)
    for i, quiz_info in enumerate(selected_chimie_quizzes):
        y_position = add_quiz_to_pdf(quiz_info, y_position, start_num_chimie + i + 1)
...
def generate_simulation():
    ...
    # Verifică dacă avem suficiente grile
    ...
    # Selectează aleatoriu grile
    selected_bio = random.sample(bio_quizzes, NUM_BIO_QUIZZES)
    selected_chimie_teorie = random.sample(chimie_teorie_quizzes, NUM_CHIMIE_TEORIE_QUIZZES)
    selected_chimie_probleme = random.sample(chimie_probleme_quizzes, NUM_CHIMIE_PROBLEME_QUIZZES)

    # Ordine specifică: biologie apoi chimie (teorie + probleme intercalate)
    selected_bio_quizzes = selected_bio # Toate grilele de biologie
    
    selected_chimie_combined = selected_chimie_teorie + selected_chimie_probleme
    random.shuffle(selected_chimie_combined) # Amestecă grilele de chimie (teorie și probleme)

    # Lista finală de grile pentru documente (Word/PDF)
    # ATENTIE: Aceasta lista nu mai este amestecata complet, ci este ordonata pe subiecte.
    # Nu se mai face random.shuffle(selected_quizzes) global.
    final_ordered_quizzes_for_docs = selected_bio_quizzes + selected_chimie_combined

    # Generează conținutul pentru fișierul TXT (pentru toate grilele selectate)
    chapter_quiz_map = {}
    for quiz in final_ordered_quizzes_for_docs: # Folosim lista ordonată pentru a colecta info
        chapter = quiz["chapter_number"]
        quiz_num = quiz["quiz_number"]
        if chapter not in chapter_quiz_map:
            chapter_quiz_map[chapter] = []
        chapter_quiz_map[chapter].append(quiz_num)
    
    chapter_quiz_map_lines = []
    for chapter in sorted(chapter_quiz_map.keys()):
        quizzes_in_chapter = sorted(chapter_quiz_map[chapter])
        chapter_quiz_map_lines.append(f"cap{chapter} -> grila {', '.join(map(str, quizzes_in_chapter))}")

    # Creează directorul de output pentru simulare
    os.makedirs(OUTPUT_SIMULATION_DIR, exist_ok=True)
    
    simulation_name = f"simulare_{random.randint(1000,9999)}"
    
    # Salvează fișierul TXT
    ...
    # Generează documentele Word și PDF
...
if __name__ == "__main__":
    if not os.path.exists(QUIZ_BASE_DIR):
        print(f"Eroare: Directorul de bază pentru grile '{QUIZ_BASE_DIR}' nu există.")
